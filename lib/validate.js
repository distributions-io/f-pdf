...
* @param {Number} [options.d1=1] - numerator degrees of freedom
* @param {Number} [options.d2=1] - denominator degrees of freedom
...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'd1' ) ) {
		...
		if ( !isPositive( opts.d1 ) ) {
			return new TypeError( 'pdf()::invalid option. `d1` parameter must be a positive number. Option: `' + opts.d1 + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'd2' ) ) {
		opts.d2 = options.d2;
		if ( !isPositive( opts.d2 ) ) {
			return new TypeError( 'pdf()::invalid option. `d2` parameter must be a positive number. Option: `' + opts.d2 + '`.' );
		}
	}
