...
describe( 'matrix pdf', function tests() {
	...
	it( 'should throw an error if provided unequal length matrices', function test() {
		...
		function badValues() {
			pdf( matrix( [10,10] ), mat, d1, d2 );
		}
	});

	it( 'should evaluate the F pdf for each matrix element', function test() {
		var actual, i;

		actual = matrix( [5,5], 'float64' );
		actual = pdf( actual, mat, d1, d2 );
	...
	it( 'should return an empty matrix if provided an empty matrix', function test() {
		...
		mat = matrix( [0,10] );
		assert.deepEqual( pdf( out, mat, d1, d2 ).data, expected );
